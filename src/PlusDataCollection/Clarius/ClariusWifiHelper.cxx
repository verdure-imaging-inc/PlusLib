...
// max time ClariusWifiHelper will wait for the Clarius network to become
// available during call to CONNECT
static const float MAX_NETWORK_READY_WAIT_TIME_SEC = 25.0;

//-------------------------------------------------------------------------------------------------
ClariusWifiHelper::ClariusWifiHelper()
  : Connected(false)
  , HClient(NULL)
  , CurApiVersion(0)
{
...
//-------------------------------------------------------------------------------------------------
PlusStatus ClariusWifiHelper::Initialize()
{
  // already initialized, return failure
  if (this->HClient != NULL)
  {
    LOG_ERROR("ClariusWifiHelper already intialized, this initialization call was rejected");
    return PLUS_FAIL;
  }
...
//-------------------------------------------------------------------------------------------------
PlusStatus ClariusWifiHelper::DeInitialize()
{
...
//-------------------------------------------------------------------------------------------------
PlusStatus ClariusWifiHelper::IsClariusNetworkReady(std::string ssid)
{
  // check ClariusWifiHelper was initialized
  if (this->HClient == NULL)
  {
    LOG_ERROR("ClariusWifiHelper must be initialized before calling IsClariusNetworkReady");
    return PLUS_FAIL;
  }
...
//-------------------------------------------------------------------------------------------------
PlusStatus ClariusWifiHelper::ConnectToClariusWifi(std::string ssid, std::string password)
{
  // already connected, return failure
  if (this->Connected)
  {
    LOG_ERROR("ClariusWifiHelper already connected to Clarius network, this connection call was rejected");
  }
...
//-------------------------------------------------------------------------------------------------
PlusStatus ClariusWifiHelper::DisconnectFromClariusWifi()
{
...
//-------------------------------------------------------------------------------------------------
PlusStatus ClariusWifiHelper::StringToSsid(std::string ssid, PDOT11_SSID pdot11_ssid)
{
